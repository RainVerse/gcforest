...
class gcForest_gdbt(object):

    def __init__(self, shape_1X=None, n_mgsRFtree=100, window=None, stride=1,
                 cascade_test_size=0.2, n_cascadeRF=2, n_cascadeRFtree=250, cascade_layer=np.inf,
        ...
        :param n_mgsRFtree: int (default=100)
        ...
        :param n_cascadeRFtree: int (default=250)
    ...
    def cascade_forest(self, X, y=None):
        ...
        if y is not None:
            ...
            feat_arr = self._create_feat_arr(X_train, prf_crf_pred_layer)
            prf_crf_pred_ref = self._gbdt_layer(feat_arr, y_train)
            ...
            if hasattr(self, 'gdbt_layer'):
                feat_arr = self._create_feat_arr(X, prf_crf_pred_ref)
                prf_crf_pred_ref = self._gbdt_layer(feat_arr)

        return prf_crf_pred_ref

    def _gbdt_layer(self, X, y=None):
        min_samples = getattr(self, 'min_samples_cascade')
        gdbt = GradientBoostingClassifier(subsample=0.8, n_estimators=250, max_depth=7, min_samples_split=min_samples)
        if y is not None:
            print('Adding/Training GDBT Layer')
            gdbt.fit(X, y)
            setattr(self, 'gdbt_layer', gdbt)
            return None
        elif y is None:
            gdbt = getattr(self, 'gdbt_layer')
            gdbt_pred = []
            gdbt_pred.append(gdbt.predict_proba(X))
            return gdbt_pred
